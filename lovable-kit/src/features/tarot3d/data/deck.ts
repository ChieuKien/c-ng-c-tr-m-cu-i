...
// Ảnh: bộ Rider–Waite–Smith bản in 1909 (Pamela Colman Smith, public domain), đặt ở public/tarot/cards/.
...
// Đường dẫn ảnh theo base của Vite (chạy được cả khi web đặt ở thư mục con)
const CARD_BASE = `${import.meta.env.BASE_URL}tarot/cards/`;
...
const SUITS: Record<Suit, { vi: string; en: string; element: string; cards: Row4[] }> = {
  wands: {
...
function build(): TarotCard[] {
  const deck: TarotCard[] = MAJOR.map(([vi, en, kUp, up, kRev, rev], i) => ({
    id: `major-${pad(i)}`,
  ...
  for (const suit of Object.keys(SUITS) as Suit[]) {
    ...
    s.cards.forEach(([kUp, up, kRev, rev], i) => {
      deck.push({
        ...
        img: `${CARD_BASE}${suit}-${pad(i + 1)}.jpg`,
        keysUp: kUp,
        up,
        keysRev: kRev,
        rev,
...
export type SpreadKey = 'daily' | 'time' | 'love' | 'career';
...
const ELEMENT_TEXT: Record<Suit, string> = {
...
export function summarize(draws: Draw[]): string[] {
  ...
  const top = (Object.entries(counts) as [Suit, number][]).sort((a, b) => b[1] - a[1])[0];
